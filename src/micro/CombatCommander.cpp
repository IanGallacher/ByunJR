...
void CombatCommander::OnStart()
{
    ...
    SquadOrder mainAttackOrder(SquadOrderTypes::Attack, sc2::Point2D(0.0f, 0.0f), 20, "Attack Enemy Base");
...
void CombatCommander::OnFrame(const std::set<const UnitInfo*> & combat_units)
{
    ...
    const SquadOrder main_attack_order(SquadOrderTypes::Attack, GetMainAttackLocation(), 20, "Attack Enemy Base");
...
sc2::Point2D CombatCommander::GetMainAttackLocation() const
{
    ...
    if (enemy_base_location)
    {
        ...
        if (!bot_.Info().Map().IsExplored(enemy_base_position))
        {
    ...
    for (const auto & kv : bot_.Info().UnitInfo().GetUnitInfoMap(sc2::Unit::Alliance::Enemy))
    {
...
//    const BaseLocation* myBaseLocation = bot_.Info().Bases().getPlayerStartingBaseLocation(Players::Self);
...
//    const BaseLocation* enemyBaseLocation = bot_.Info().Bases().getPlayerStartingBaseLocation(Players::Enemy);
//    for (const BaseLocation* myBaseLocation : bot_.Info().Bases().getOccupiedBaseLocations(Players::Self))
//    {
